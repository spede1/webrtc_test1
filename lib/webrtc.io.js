...
rtc.fire = function(eventName, _) {
  console.log("rtc.fire, eventName: " + JSON.stringify(eventName));
...
function attachEvents(manager) {
  console.log("- webrtc.io, attachEvents");
  
  manager.on('connection', function(socket) {
    console.log("mangager.on connection");
  ...
  rtc.on('join_room', function(data, socket) {
    console.log("- webrtc.io, join_room, data.room: " + JSON.stringify(data.room));
  ...
  rtc.on('send_ice_candidate', function(data, socket) {
    console.log('webrtc.io, send_ice_candidate');
  ...
  rtc.on('send_offer', function(data, socket) {
    console.log('webrtc.io, send_offer, data: ' + JSON.stringify({
          "sdp": data.sdp,
          "socketId": socket.id
      }));
    iolog('send_offer');
  ...
  rtc.on('send_answer', function(data, socket) {
    console.log("webrtc.io, rtc.on send_answer, data.sdp:" + JSON.stringify({
          "sdp": data.sdp,
          "socketId": socket.id
        }));
...
rtc.getSocket = function(id) {
  console.log("webrtc.io, rtc.getSocket, for id: " + id);
  ...
  console.log("webrtc.io, rtc.getSocket, connections length: " + connections.length);
